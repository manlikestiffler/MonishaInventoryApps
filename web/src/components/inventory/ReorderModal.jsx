...
const ReorderModal = ({ isOpen, onClose, variant, size, currentStock, reorderLevel, batchId, product }) => {
  ...
  const [selectedCurrentStock, setSelectedCurrentStock] = useState(currentStock || 0);
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setSelectedCurrentStock(currentStock || 0);
      setSelectedReorderLevel(reorderLevel || 5);
      // Set default reorder quantity to 2x reorder level
      setQuantityToReorder((reorderLevel || 5) * 2);
      // Fetch batch inventory stock
      fetchBatchStock();
    }
  }, [isOpen, currentStock, reorderLevel, selectedVariant, selectedSize]);
  ...
  const fetchBatchStock = async () => {
    ...
    try {
      ...
      console.log('Looking for variant:', { variantType: selectedVariant.variantType, color: selectedVariant.color, size: selectedSize });
      ...
      if (matchingItem) {
        ...
        if (matchingSize) {
          ...
          console.log('Found batch stock:', matchingSize.quantity, 'for size', selectedSize);
        } else {
